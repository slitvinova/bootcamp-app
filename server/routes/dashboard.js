...
router.get('/metrics', async (req, res) => {
  try {
    const [{ total: totalTestCases }, allBugs, allRuns, recentActivity] = await Promise.all([
      db.list({ limit: 1 }),
      db.bugs.list(),
      db.testRuns.list(),
      db.bugs.recentActivity(10),
    ]);
...
router.get('/trends', async (req, res) => {
  try {
    const [allRuns, allBugs, allActivity, { rows: allCases }] = await Promise.all([
      db.testRuns.list(),
      db.bugs.list(),
      db.bugs.recentActivity(9999),
      db.list({ limit: 99999 }),
    ]);
