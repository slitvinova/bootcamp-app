...
router.get('/', (req, res) => {
  ...
  const bugs = db.bugs.list({ status, severity, priority, search, sort, order });
...
router.get('/:id', (req, res) => {
  const bug = db.bugs.get(req.params.id);
  if (!bug) return res.status(404).json({ success: false, data: null, error: 'Bug not found.' });
  const activity = db.bugs.getActivity(req.params.id);
...
router.put('/:id', (req, res) => {
  ...
  const updated = db.bugs.update(req.params.id, {
...
router.delete('/:id', (req, res) => {
  const removed = db.bugs.remove(req.params.id);
...
router.patch('/:id/status', (req, res) => {
  ...
  const updated = db.bugs.changeStatus(req.params.id, status, message, existing.status);
  const activity = db.bugs.getActivity(req.params.id);
...
router.post('/:id/comments', (req, res) => {
  ...
  db.bugs.addComment(req.params.id, message.trim());
  const activity = db.bugs.getActivity(req.params.id);
  res.json({ success: true, data: { ...bug, activity }, error: null });
});
