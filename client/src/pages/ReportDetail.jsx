...
export default function ReportDetail() {
  ...
  if (error) return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center text-red-500 text-sm">{error}</div>
  );
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-slate-800 rounded border border-slate-700 p-6 mb-4">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <p className="text-xs text-slate-400 mb-1">Report #{report.id} &nbsp;·&nbsp; Run #{report.run_id}</p>
              <h1 className="text-xl font-semibold text-white mb-1">{report.suite_name}</h1>
              <p className="text-xs text-slate-400">Run {formatDate(report.run_date)} &nbsp;·&nbsp; Generated {formatDate(report.generated_at)}</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => window.print()}
                className="px-3 py-2 text-xs font-medium text-slate-200 border border-slate-600 rounded-md hover:bg-slate-900"
              >
                Print / Save as PDF
              </button>
              <a
                ...
                download
                className="px-3 py-2 text-xs font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 inline-flex items-center gap-1"
              >
                Download HTML
              </a>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {[
            { label: 'Total',   value: report.total_count,   color: 'text-white' },
            ...
            <div key={label} className="bg-slate-800 rounded border border-slate-700 p-4 text-center">
        ...
        <div className="bg-slate-800 rounded border border-slate-700 p-4 mb-6">
          ...
          <p className="text-xl font-semibold text-white">{passRate}%</p>
        </div>

        {/* Results table */}
        <div className="bg-slate-800 rounded border border-slate-700 overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-700">
            <h2 className="text-sm font-medium text-slate-200">Test Results</h2>
          </div>
          {report.results.length === 0 ? (
            <p className="px-4 py-8 text-center text-sm text-slate-400">No results in this report.</p>
          ) : (
                ...
                {report.results.map((r, i) => (
                  <tr key={i} className={r.result === 'failed' ? 'bg-red-900/40' : ''}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-block px-1.5 py-0.5 rounded text-xs font-medium ${SEVERITY_BADGE[r.severity] || 'bg-slate-700 text-slate-500'}`}>
                        {r.severity}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      {r.result ? (
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium capitalize ${RESULT_BADGE[r.result] || 'bg-slate-700 text-slate-500'}`}>
                          {r.result}
                        </span>
                      ) : (
