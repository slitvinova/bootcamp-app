      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded border border-stone-200 p-6 mb-4">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-stone-400 mb-1">Run #{run.id}</p>
              <h1 className="text-xl font-semibold text-stone-900 mb-2">{run.suite_name}</h1>
        ...
        <div className="bg-white rounded border border-stone-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-stone-200">
            <h2 className="text-sm font-medium text-stone-700">Test Cases</h2>
          </div>
          <ul className="divide-y divide-stone-100">
            {run.results.map((r, i) => (
              <li key={r.id} className="px-4 py-4">
                <div className="flex items-start gap-3">
                  <span className="text-xs text-stone-400 w-5 text-right flex-shrink-0 mt-0.5">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span className="text-sm font-medium text-stone-900">{r.test_case_title}</span>
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full border border-stone-200 rounded-md px-3 py-1.5 text-xs text-stone-700 focus:outline-none focus:ring-2 focus:ring-orange-300 resize-none mb-2"
                    />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {saving[r.id] && <span className="text-xs text-stone-400">Saving…</span>}
                    </div>
                  </div>
                </div>
              </li>
