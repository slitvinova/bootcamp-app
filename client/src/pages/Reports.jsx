      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {!error && reports.length === 0 ? (
          <div className="bg-slate-800 rounded border border-slate-700 py-16 text-center">
            ...
            <p className="text-slate-300 text-xs mt-1">Open a completed test run and click Generate Report.</p>
          </div>
        ) : (
                ...
                {reports.map(r => (
                  <tr key={r.id} className="hover:bg-slate-900">
                    <td className="px-4 py-3">
                      <p className="text-sm font-medium text-white">{r.suite_name}</p>
                      <p className="text-xs text-slate-400">Run #{r.run_id} &nbsp;·&nbsp; Report #{r.id}</p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2 text-xs">
                        ...
                        <span className="text-slate-400">/ {r.total_count}</span>
                      </div>
                    </td>
