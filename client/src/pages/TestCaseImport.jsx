  ...
  if (step === 'done') {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="bg-slate-800 rounded border border-slate-700 p-10 text-center max-w-sm w-full">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button
              onClick={reset}
              className="px-4 py-2 border border-slate-600 rounded-md text-sm text-slate-300 hover:bg-slate-900"
            >
              Import another
            </button>
          </div>
    ...
    <div className="min-h-screen bg-slate-900">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-semibold text-white">Import Test Cases</h1>
            <p className="text-sm text-slate-500 mt-0.5">Bulk-import test cases from a CSV file.</p>
          </div>
          <button onClick={() => navigate('/test-cases')} className="text-sm text-slate-400 hover:text-slate-300">
            ← Back to Test Cases
          </button>
        </div>
        ...
        {/* ── Step 1: Upload ──────────────────────────────────────────────── */}
        {step === 'upload' && (
          <div className="bg-slate-800 rounded border border-slate-700 p-8">
            <div
        ...
        {step === 'preview' && preview && (
          <>
            {/* Summary + actions */}
            <div className="bg-slate-800 rounded border border-slate-700 px-4 py-3 mb-4 flex items-center justify-between">
              ...
              <table className="min-w-full text-sm divide-y divide-slate-700">
                ...
                <tbody className="divide-y divide-slate-700">
                  {preview.rows.map(row => (
                    <tr key={row.row_num} className={row.valid ? 'hover:bg-slate-900' : 'bg-red-900/40'}>
                      <td className="px-4 py-3 text-xs text-slate-400">{row.row_num}</td>
                      ...
                      <td className="px-4 py-3">
                        {row.data.severity
                          ? <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${SEVERITY_BADGE[row.data.severity] || 'bg-slate-700 text-slate-500'}`}>{row.data.severity}</span>
