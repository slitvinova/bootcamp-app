...
export default function BugDetail() {
  ...
  if (!bug)      return (
    <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center gap-3">
      <p className="text-stone-400">Bug not found.</p>
      <button onClick={() => navigate('/bugs')} className="text-sm text-orange-600 hover:text-orange-800">← Back to bug list</button>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded border border-stone-200 p-6 mb-4">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-stone-400 mb-1">#{bug.id}</p>
              <h1 className="text-xl font-semibold text-stone-900 mb-3">{bug.title}</h1>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {bug.environment && <span className="text-xs text-stone-400">{bug.environment}</span>}
              </div>
              {bug.github_issue_url && (
                <a href={bug.github_issue_url} target="_blank" rel="noreferrer"
                  className="inline-flex items-center gap-1 text-xs text-orange-600 hover:text-orange-800 mt-2">
              ...
              <p className="text-xs text-stone-400 mt-2">Reported {formatDate(bug.created_at)}</p>
            </div>
            <div className="flex flex-col items-end gap-2 flex-shrink-0">
              <div className="flex items-center gap-2">
                <button onClick={() => setEditModalOpen(true)}
                  className="px-3 py-1.5 border border-stone-300 rounded-md text-sm text-stone-600 hover:bg-stone-50">Edit</button>
        ...
        {/* Details card */}
        <div className="bg-white rounded border border-stone-200 p-6 mb-4 space-y-5">
          <div>
            <h2 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1">Description</h2>
            <p className="text-sm text-stone-800 whitespace-pre-wrap">{bug.description}</p>
          </div>

          {bug.steps_to_reproduce && bug.steps_to_reproduce.length > 0 && (
            <div>
              <h2 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-2">Steps to Reproduce</h2>
              <ol className="list-decimal list-inside space-y-1">
                {bug.steps_to_reproduce.map((step, i) => (
                  <li key={i} className="text-sm text-stone-800">{step}</li>
                ))}
              </ol>
            </div>
          )}
          ...
          {(bug.expected || bug.actual) && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {bug.expected && (
                <div>
                  <h2 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1">Expected</h2>
                  <p className="text-sm text-stone-800">{bug.expected}</p>
                </div>
              )}
              {bug.actual && (
                <div>
                  <h2 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1">Actual</h2>
                  <p className="text-sm text-stone-800">{bug.actual}</p>
                </div>
              )}
            </div>
          )}
        ...
        {allowed.length > 0 && (
          <div className="bg-white rounded border border-stone-200 p-6 mb-4">
            <h2 className="text-sm font-semibold text-stone-700 mb-3">Change Status</h2>
            <form onSubmit={handleStatusChange} className="space-y-3">
              ...
              <input type="text" value={statusMessage} onChange={e => setStatusMessage(e.target.value)}
                ...
                className="w-full border border-stone-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
            </form>
          </div>
        )}
        ...
        {/* Add comment */}
        <div className="bg-white rounded border border-stone-200 p-6 mb-4">
          <h2 className="text-sm font-semibold text-stone-700 mb-3">Add Comment</h2>
          <form onSubmit={handleAddComment} className="space-y-2">
            ...
            <textarea value={comment} onChange={e => setComment(e.target.value)}
              ...
              className="w-full border border-stone-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 resize-none" />
        ...
        {bug.activity && bug.activity.length > 0 && (
          <div className="bg-white rounded border border-stone-200 p-6">
            <h2 className="text-sm font-semibold text-stone-700 mb-4">Activity</h2>
            <ul className="space-y-4">
              {bug.activity.map(a => (
                <li key={a.id} className="flex gap-3">
                  <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${a.action === 'status_change' ? 'bg-orange-400' : 'bg-stone-300'}`} />
                  <div className="flex-1">
                    {a.action === 'status_change' ? (
                      ...
                      <p className="text-sm font-medium text-stone-700">Comment</p>
                    )}
                    {a.message && <p className="text-sm text-stone-500 mt-0.5">{a.message}</p>}
                    <p className="text-xs text-stone-400 mt-0.5">{formatDate(a.timestamp)}</p>
                  </div>
                </li>
              ))}
            </ul>
