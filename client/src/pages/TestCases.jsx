...
function SortIcon({ field, sortField, sortOrder }) {
  if (sortField !== field) return <span className="ml-1 text-slate-600 dark:text-slate-300">↕</span>;
    ...
    <div className="min-h-screen bg-white dark:bg-slate-900">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">Test Cases</h1>
          ...
          <div className="flex gap-2">
            <a
              ...
              className="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors"
            >
              Export CSV
            </a>
            <button
        ...
        <div className="flex gap-3 mb-4 flex-wrap">
          <input
            ...
            className="border border-slate-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white dark:bg-slate-800"
          />
          <select
            value={filterStatus}
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <th
                  className="text-left px-4 py-3 font-medium text-slate-600 dark:text-slate-300 cursor-pointer select-none whitespace-nowrap"
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {loading ? (
                <tr>
                  <td colSpan={5} className="text-center py-12 text-slate-500 dark:text-slate-400">Loading…</td>
                </tr>
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center py-12 text-slate-500 dark:text-slate-400">No test cases found.</td>
                </tr>
              ) : (
                rows.map(tc => (
                  <tr key={tc.id} className="hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <button
                        onClick={() => openEdit(tc)}
                        className="text-slate-500 dark:text-slate-400 hover:text-blue-400 mr-3 transition-colors"
                        title="Edit"
                        aria-label="Edit"
                      >
                        ✎
                      ...
                      <button
                        ...
                        className="text-slate-500 dark:text-slate-400 hover:text-red-400 transition-colors"
                        title="Delete"
                        aria-label="Delete"
                      >
                        ✕
          ...
          <div className="flex items-center justify-between mt-4 text-sm text-slate-600 dark:text-slate-300">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-1 border border-slate-300 dark:border-slate-600 rounded-md disabled:opacity-40 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
              >
                ← Prev
              </button>
