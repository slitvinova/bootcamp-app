    ...
    <div className="min-h-screen bg-stone-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-stone-900">Test Runs</h1>
          <button
            onClick={openModal}
        ...
        {runs.length === 0 ? (
          <div className="bg-white rounded border border-stone-200 py-16 text-center">
            <p className="text-stone-400 text-sm">No test runs yet.</p>
            <p className="text-stone-400 text-xs mt-1">Click "+ New Run" to start one.</p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-stone-200">
              <thead className="bg-stone-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wide">Started</th>
              ...
              <tbody className="divide-y divide-stone-100">
                {runs.map(run => (
                  <tr key={run.id} className="hover:bg-stone-50">
                    ...
                    <td className="px-4 py-3 text-sm text-stone-500">{formatDate(run.start_time)}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded shadow-xl w-full max-w-sm p-6">
            <h2 className="text-base font-semibold text-stone-900 mb-4">New Test Run</h2>

            {createError && (
              <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded px-3 py-2 mb-3">{createError}</p>
            )}

            <label className="block text-xs font-medium text-stone-600 mb-1">Suite</label>
            <select
              value={selectedSuite}
              onChange={e => setSelectedSuite(e.target.value)}
              className="w-full border border-stone-300 rounded-md px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-orange-400 mb-5"
            >
              {suites.map(s => (
