...
export default function FlakyTests() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center text-red-500 text-sm">
        {error}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-white">Flaky Tests</h1>
            <p className="text-sm text-slate-500 mt-0.5">Tests that pass in some runs and fail in others</p>
          </div>
          {total_runs > 0 && (
            <span className="text-sm text-slate-500">
              <span className="font-semibold text-white">{total_flaky}</span> test{total_flaky !== 1 ? 's' : ''} flagged across{' '}
              <span className="font-semibold text-white">{total_runs}</span> run{total_runs !== 1 ? 's' : ''}
            </span>
          )}
        </div>
        ...
        {leaderboard.length === 0 ? (
          <div className="bg-slate-800 border border-slate-700 rounded py-16 text-center">
            <p className="text-slate-400 text-sm">No flaky tests detected yet.</p>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-700">
                {leaderboard.map((test, idx) => (
                  <tr key={test.test_case_id} className="hover:bg-slate-900 transition-colors">
                    <td className="px-4 py-3 text-slate-400 font-mono text-xs">{idx + 1}</td>
                    <td className="px-4 py-3">
                      <span className="font-medium text-white">{test.test_case_title}</span>
                    ...
                    <td className="px-4 py-3 max-w-xs">
                      {test.hypothesis ? (
                        ...
                        <span className="text-slate-300 text-xs italic">Generating…</span>
                      )}
                    </td>
                  </tr>
                ))}
            ...
            <div className="px-4 py-2.5 border-t border-slate-700 bg-slate-900 flex items-center gap-3">
              <span className="text-xs text-slate-400">Dot key:</span>
